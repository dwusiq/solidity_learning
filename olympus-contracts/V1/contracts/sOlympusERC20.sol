...
/**
* 关于sOHM有需要关注的点：
* 1、协议内计算份额是用gon,但从外面用户的角度都是取sOHM的份额（即：sOHM对外，gon对内）
* 2、三者关系比较重要：sOHM份额=gon份额/_gonsPerFragment，其中_gonsPerFragment跟_totalSupply成反比，_gonsPerFragment表示每单位sOHM价值多少gon
 */
contract sOlympus is ERC20Permit, Ownable {
  ...
  struct Rebase {
    uint256 epoch; // 当前周期(epoch)的起始区块
  ...
  uint256 public INDEX; //当前gon值，用于在变基时计算当前sOHM份额
  ...
  //TOTAL_GONS是INITIAL_FRAGMENTS_SUPPLY的倍数【 TOTAL_GONS is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _gonsPerFragment is an integer.】
  //初始gon的总供应量（这个值不会变化）=uint256Max - (uint256Max%sohmTotalSupply)=uint256支持的sohmTotalSupply的最大整倍数
  uint256 private constant TOTAL_GONS =
    MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  //sOHM的最大供应量【 MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_GONS + 1) - 1) / 2】
  ...
  /**
   *  @notice 初始化参数（只允许初始化一次）
   *  @param stakingContract_ address  staking合约地址
   *  @return bool  是否成功
   */
  ...
  /**
   *  @notice 变基，使staking合约中OHM与sOHM的份额相等【increases sOHM supply to increase staking balances relative to profit_】
    ...
    returns (uint256)
  {
    ...
    //计算每单位sOHM相应价值多少GONS
    //间接给OHM质押者分红（由接口balanceOf得知，_gonsPerFragment越小，则同样的gon得到的sOHM越多）
    _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
  ...
  function circulatingSupply() public view returns (uint256) {
    //返回值=sOHM总份额-staking合约持有的sOHM份额
