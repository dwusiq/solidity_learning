...
contract MockOlympusBondDepository is Ownable {
    ...
    address public staking; // 用于自动质押债券的支出费用【to auto-stake payout】
    address public stakingHelper; // to stake and claim if no staking warmup
    bool public useHelper; //是否使用helper,  true:使用stakingHelper false:使用staking
    ...
    struct Terms {
        uint256 controlVariable; // 债券价格的缩放比例变量【scaling variable for price】
        uint256 vestingTerm; // 债券锁仓时间包含几个区块【in blocks】
        uint256 minimumPrice; // 债券最低允许价格【vs principle value】
        uint256 maxPayout; // 债券单笔最大支出【in thousandths of a %. i.e. 500 = 0.5%】
        uint256 fee; // 购买债券的手续费【as % of bond payout, in hundreths. ( 500 = 5% = 0.05 for every 1 paid)】
        uint256 maxDebt; // 最大允许用户持有的债券总额【9 decimal debt ratio, max % total supply created as debt】
    }
    ...
    function deposit(
        ...
        //计算支付的token份额价值多少OHM(也就是将token自己精度的份额转化为OHM精度的份额)
        uint256 value = ITreasury(treasury).valueOfToken(principle, _amount);
        //计算应付给用户的OHM份额
        uint256 payout = payoutFor(value); // payout to bonder is computed
        console.log("priceInUSD:%s", priceInUSD);
        console.log("nativePrice:%s", nativePrice);
    ...
    /**
     *  @notice 计算债券比例【calculate current ratio of debt to OHM supply】
    ...
    /**
     *  @notice 剩余需要合约支付的债券份额【calculate debt factoring in decay】
     *  @return uint
     */
