...
contract OlympusStaking is Ownable {
  ...
  struct Epoch {
    uint256 length; //每个周期(Epoch)包含多少个区块
    uint256 number; //当前周期的起始区块
    uint256 endBlock; //当前周期将在哪个区块结束
    uint256 distribute;
  }
  Epoch public epoch;
  ...
  mapping(address => Claim) public warmupInfo; //处于热身周期的信息（热身结束前赎回则没有收益）

  /**
        @notice 用户质押OHM进入热身期【stake OHM to enter warmup】
        @param _amount uint
        @return bool
     */
  function stake(uint256 _amount, address _recipient) external returns (bool) {
    ...
    warmupInfo[_recipient] = Claim({
      ...
      expiry: epoch.number.add(warmupPeriod), //所有用户的热身期结束区块=当前周期(epoch)的起始区块+热身时长
      lock: false
    });

    //将sOHM转给warmupContract合约
    IERC20(sOHM).safeTransfer(warmupContract, _amount);
    return true;
  }

  /**
    @notice 热身期结束后，用户提取报酬(sOHM),没有OHM【retrieve sOHM from warmup】
    @param _recipient address
     */
  function claim(address _recipient) public {
    Claim memory info = warmupInfo[_recipient];
    //要求用户质押时的那个周期(epoch)已结束（即已开启第二个周期）
    if (epoch.number >= info.expiry && info.expiry != 0) {
      //删除用户的质押信息，并给用户支付报酬（sOHM）。
  ...
  /**
    @notice 不关注是否已过热身期，用户只取回自己质押的OHM(用户的收益会被扣留给staking合约)【forfeit sOHM in warmup and retrieve OHM】
   */
  ...
  /**
   * @notice 切换热身状态数据的锁的状态【prevent new deposits to address (protection from malicious activity)】
   */
  ...
  /**
        @notice 用sOHM兑换OHM【redeem sOHM for OHM】
        @param _amount uint   提取的sOHM份额
        @param _trigger bool  true：触发变基  false:不触发变基
     */
  ...
  /**
        @notice 返回跟踪变基增长的sOHM指数  【returns the sOHM index, which tracks rebase growth】
        @return uint
     */
  ...
  /**
        @notice 获取当前合约持有多少OHM【returns contract OHM holdings, including bonuses provided】
        @return uint
     */
  ...
  /**
   * @notice 为新参与者设置热身时间（需要超过这么多区块才能领取收益）【set warmup period for new stakers】
